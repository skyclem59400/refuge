...
| 2026-06-02 | **Générateur IA de descriptifs — species-aware (remontée Céline « bloquée pour les chats »)** : le générateur (`/api/ai/generate-description`) ne disposait que de 2 few-shot **chiens** (Amaya, Tupac) + un system prompt à dominante canine → pour un chat, le modèle plaquait le registre chien (promenade, laisse, puces 🐶) et sortait une fiche inexploitable. Ajout de 2 few-shot **félins** authentiques (vraies fiches de prod Virgil + Ayesha, email normalisé `accueil@sda-nord.com`) + addendum `CAT_GUIDANCE` injecté uniquement pour les chats (vocabulaire félin imposé, CTA en 😺/🐱, rubrique éducation adaptée litière/griffoir, profil humain intérieur). Sélection few-shot conditionnée par `species`. Chemin chien **inchangé** (zéro régression). Validé par génération live sur un chat test : fiche 1ʳᵉ personne, 3442 car., vocabulaire félin, CTA 😺, mention légale OK. | TS check ✅ + test génération live ✅ |
| 2026-06-03 | **Fix publication site : Mamie Simone (foster_family) invisible sur sda-nord.com** (remontée Céline « bloquée pour les chats »). Diagnostic : la policy RLS Supabase `Public can view adoptable animals` limitait l'anon-key à `status='shelter'` malgré l'élargissement du code (`/animaux/page.tsx`) à `foster_family` la veille. Une 2e policy `animals_public_select_sheltered` couvrait bien `foster_family` mais était attachée à `{-}` (aucun rôle), donc inerte. Fix : `ALTER POLICY "Public can view adoptable animals" USING (adoptable = true AND status IN ('shelter','foster_family'))` + `DROP POLICY "animals_public_select_sheltered"`. Impact : +6 animaux en accueil temporaire deviennent visibles côté site (Mamie Simone incluse). `adoptable=true` reste obligatoire. | migration `rls_animals_public_include_foster_family` appliquée via MCP |
| 2026-06-04 | **Fix diagramme Tendance Sorties — barres invisibles en dark mode** : la barre de tendance et la barre « Sorties par personne » utilisaient `bg-primary`. Or en dark mode `--color-primary` (navy `#1e3a5f`) = `--color-surface`, donc barres rendues mais invisibles sur le card. Pattern identique au bug `text-primary` des donations corrigé précédemment, mais cette fois sur des **fonds** (le contournement CSS `html.dark .text-primary { color: teal }` ne couvre que les textes). Remplacé `bg-primary` par `bg-teal` sur les 2 barres du composant `outing-stats.tsx`. | commit `f492230` |
| 2026-06-04 | **Fix certificat d'engagement — bloc Refuge vide + signature hors encart** (remontée Clément sur le certif signé par Frédérique Senechal le 03/06). Deux bugs : <ul><li>**Bloc Refuge** : seul « Pour SDA » affiché, pas de tampon SVG ni de nom du salarié. Le contrat d'adoption avait déjà le bon pattern (`is-refuge` + `buildCachetSvg` + `createdByName`) mais le template engagement créé plus tard ne l'avait pas. Réutilisé `buildCachetSvg` + récupération du nom via `get_users_info(created_by)`.</li><li>**Signature adoptant en bas de page** : les `pageY` Documenso (79/84/89) ciblaient le bas absolu de la dernière page mais la sigbox du template tombait à un autre endroit selon le contenu (le certif fait 3 pages). Fix : `page-break-before: always` sur `.signed-at` pour forcer signed-at + signatures + footer sur une page dédiée, `min-height` sigbox-body 32→40mm, et `pageY` recalibrés à 12/16/20 (au lieu de 79/84/89) pour viser le créneau réel de la sigbox adoptant sur cette nouvelle dernière page.</li></ul>Fix valable pour les **prochains** certificats (les déjà signés restent figés chez Documenso). | commit `8cf2676` |

...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const conn of connections) {
      const now = new Date()
      const startDate = conn.sync_cursor
        ? new Date(conn.sync_cursor)
      ...
      while (offset < 9000) {
        const response = await fetch(`${RINGOVER_API_BASE}/calls`, {
          ...
          body: JSON.stringify({
            limit_count: 500,
            limit_offset: offset,
            filter_date_start: startDate.toISOString(),
            filter_date_end: now.toISOString(),
        ...
        const callList = apiBody?.call_list || []
        if (!Array.isArray(callList) || callList.length === 0) break

        const records = callList.map((call: Record<string, unknown>) => {
          const dir = call.direction === 'out' ? 'out' : 'in'
          const st = String(call.status || call.call_status || 'UNKNOWN')
          return {

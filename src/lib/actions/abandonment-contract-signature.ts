...
export async function sendAbandonmentContractForSignature(contractId: string) {
  try {
    ...
    // ─────────────────────────────────────────────────────────────────────
    // RENVOI PUR (doc Documenso déjà créé) : juste renvoyer l'email Brevo.
    // ─────────────────────────────────────────────────────────────────────
    if (contract.signature_status === 'pending' && contract.documenso_document_id) {
      ...
      try {
        ...
        console.warn('[abandonment-contract-signature] resend: refresh signing URL failed:', (e as Error).message)
      }
      ...
      logActivity({
        ...
        details: { resent: true, recipient_email: contract.cedant.email },
      })
